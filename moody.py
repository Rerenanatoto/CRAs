...
def interpolate_quant(value, ranges, higher_is_better):
    ...
    Cada rating no índice i tem score central = i+1.
    Dentro de cada faixa, o score varia linearmente de i+0.5 a i+1.5.

    Para higher_is_better=True:
      valor no topo da faixa (máximo) → melhor → score = i+0.5
      valor na base da faixa (mínimo) → pior → score = i+1.5

    Para higher_is_better=False:
      valor na base da faixa (mínimo) → melhor → score = i+0.5
      valor no topo da faixa (máximo) → pior → score = i+1.5
    """
    INF = 90000
    n = len(ranges)
    for i, (rating, rlo, rhi) in enumerate(ranges):
        score_best = i + 0.5   # melhor extremo da faixa
        score_worst = i + 1.5  # pior extremo da faixa

        is_open_lo = (abs(rlo) >= INF)
        is_open_hi = (abs(rhi) >= INF)

        if higher_is_better:
            if is_open_hi:  # faixa aberta no topo (ex: aaa ≥ 5.7)
                if value >= rlo:
                    return float(i + 1)
                continue
            if is_open_lo:  # faixa aberta na base (ex: ca < -1.0)
                if value <= rhi:
                    return float(i + 1)
                continue
            if value >= rhi:
            ...
            if span == 0:
                return float(i + 1)
            frac = (rhi - value) / span  # 0=topo(melhor), 1=base(pior)
            return score_best + frac * (score_worst - score_best)

        else:  # lower is better
            if is_open_lo:  # faixa aberta na base (ex: aaa ≤ 5)
                if value <= rhi:
                    return float(i + 1)
                continue
            if is_open_hi:  # faixa aberta no topo (ex: ca > 150)
                if value >= rlo:
                    return float(i + 1)
                continue
            if value < rlo:
            ...
            if span == 0:
                return float(i + 1)
            frac = (value - rlo) / span  # 0=base(melhor), 1=topo(pior)
            return score_best + frac * (score_worst - score_best)
...
# Fonte: Exhibit 4, pág. 7 do PDF / Factor 1 - Table 1 do Excel
# Formato: (rating, limite_inferior, limite_superior)
# ═══════════════════════════════════════════════════════════════════════════

F1_GDP_GROWTH = [  # HIGHER IS BETTER
...
F1_NOM_GDP = [  # HIGHER IS BETTER (US$ bilhões)
...
F1_GDP_PC = [  # HIGHER IS BETTER (US$ PPP)
...
# Pesos Fator 1 (Exhibit 4): Growth Dynamics 0.35 (GDP Growth 0.25 + MAD Vol 0.10),
#   Scale 0.30, National Income 0.35
F1_W_GDP = 0.25
F1_W_MAD = 0.10
F1_W_NOM = 0.30
F1_W_PC  = 0.35
...
# Fonte: Exhibit 5 pág. 25 / Factor 3 - Table 1 do Excel
# Pesos: cada métrica 25%
# ═══════════════════════════════════════════════════════════════════════════

F3_GGGD_GDP = [  # LOWER IS BETTER (%)
...
# ═══════════════════════════════════════════════════════════════════════════
# FATOR 4 – Matriz de Risco do Setor Bancário (Exhibit 13, pág. 45)
# Linhas: Total Bank Assets/GDP | Colunas: BSCE agrupado
# ═══════════════════════════════════════════════════════════════════════════

BSR_MATRIX = [
    # aaa-a3  baa1   baa2   baa3   ba1-ba2  ba3-b3  caa-ca
    ["a",    "a",   "baa",  "ba",  "b",     "b",    "ca"],    # ≥400%
    ["a",    "a",   "baa",  "baa", "ba",    "b",    "ca"],    # 230-400%
    ["a",    "a",   "a",    "baa", "ba",    "ba",   "b"],     # 180-230%
    ["a",    "a",   "a",    "a",   "baa",   "ba",   "ba"],    # 80-180%
    ["aaa",  "aa",  "aa",   "a",   "a",     "baa",  "ba"],    # <80%
]

def bsce_to_col(r):
    """Mapeia rating BSCE para coluna da matriz BSR."""
    idx = RATING_SCALE.index(r) if r in RATING_SCALE else 19
    if idx <= 6:   return 0   # aaa-a3
    if idx == 7:   return 1   # baa1
    if idx == 8:   return 2   # baa2
    if idx == 9:   return 3   # baa3
    if idx <= 11:  return 4   # ba1-ba2
    if idx <= 15:  return 5   # ba3-b3
    return 6                   # caa-ca
...
# Linhas = Economic Resiliency (aaa a ca)
# Colunas = Fiscal Strength (aaa a ca)
# ═══════════════════════════════════════════════════════════════════════════
...
# Linhas = SETR alpha | Colunas = GFS alfanumérico (aaa a caa1 = 17 cols)
# ═══════════════════════════════════════════════════════════════════════════
...
def calc_factor3(gggd_gdp, gggd_rev, int_rev, int_gdp,
    ...
    # Ajuste: Mudança Histórica Dívida/GDP (p.p., t-8 a t)
    a_hist = 0
    if hist_chg >= 50:    a_hist = -2
    elif hist_chg >= 25:  a_hist = -1

    # Ajuste: Mudança Esperada Dívida/GDP (p.p., t a t+2)
    a_exp = 0
    if exp_chg < -5:      a_exp = 1     # < (5) → +1
    elif exp_chg <= 5:    a_exp = 0     # (5) - 5 → 0
    elif exp_chg <= 10:   a_exp = -1    # 5 - 10 → -1
    elif exp_chg <= 15:   a_exp = -2    # 10 - 15 → -2
    elif exp_chg >= 50:   a_exp = -3    # ≥ 50 → -3
    else:                 a_exp = -2    # 15-50 permanece -2
...
def calc_factor4(political, ease_access, high_refin,
    ...
    # Sub-fator 1: Political Risk
    pol_score = broad_to_score(political)

    # Sub-fator 2: Government Liquidity Risk
    ease_score = broad_to_score(ease_access)
    liq_score = clamp_score(ease_score + high_refin)  # high_refin = 0,1,2 (piora)
    liq_alpha = score_to_broad(liq_score)

    # Sub-fator 3: Banking Sector Risk
    ...
    # Sub-fator 4: External Vulnerability Risk
    ...
    # SETR = pior (maior score) dos 4 sub-fatores
...
st.set_page_config(
    page_title="Moody's Sovereign Rating Model",
    layout="wide",
    page_icon="🏛️",
)
...
st.sidebar.markdown("**Metodologia de Rating Soberano**")
st.sidebar.markdown("---")
...
# ═══════════════════════════════════════════════════════════════════════════
# DEFAULTS – Brasil 2024 (armazenados em session_state)
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# PÁGINA: FACTOR 1 – ECONOMIC STRENGTH
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# PÁGINA: FACTOR 2 – INSTITUTIONS & GOVERNANCE
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# PÁGINA: FACTOR 3 – FISCAL STRENGTH
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
# PÁGINA: FACTOR 4 – SUSCEPTIBILITY TO EVENT RISK
# ═══════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    # Usar resultados armazenados ou calcular com defaults do Brasil
    ...
    # Linha 1: Fatores 1-4 + Economic Resiliency
